 ...
 * generated by the edger8r tool. Ideally we'd simply include the
 * 'tcti_sgx_t.h' header but we can't: the edger8r refuses to generate
 * edge routines for an edl file that has only 'trusted' functions.
 * We don't need to link against these symbols when building
 * libtss2-tcti-sgx.a so this is really just a cosmetic issue of
 * keeping the compiler from complaining about implicit declarations.
 */
sgx_status_t tcti_sgx_init_ocall (uint64_t *session_id);
sgx_status_t tcti_sgx_transmit_ocall (TSS2_RC *rc,
                                      uint64_t session_id,
                                      size_t size,
                                      const uint8_t *command);
sgx_status_t tcti_sgx_receive_ocall (TSS2_RC *rc,
                                     uint64_t session_id,
                                     size_t size,
                                     uint8_t *response,
                                     int32_t timeout);
...
sgx_status_t tcti_sgx_cancel_ocall (TSS2_RC *rc,
                                    uint64_t session_id);
sgx_status_t tcti_sgx_set_locality_ocall (TSS2_RC *rc,
                                          uint64_t session_id,
                                          uint8_t locality);
